...
- `context.md` 维护接手摘要、最终目标、当前目标、当前阶段、阻塞、下一步、开放风险和过期条件。接手摘要用于让新会话或接手人先定位现场，使用固定表格记录工作根目录、相关子仓库或工作区、当前分支/commit、涉及源码目录、源码侧说明文档路径、当前阶段和立即下一步。最终目标单独成节，避免被埋在摘要里。只写导航级摘要，不复制 README、实施计划、命令全文或案例细节。
...
只放接手所需的导航级摘要；详细事实、命令、案例和项目文档分别放到对应主记录。
...
- `context.md` 可以让新会话快速恢复现场：知道工作目录、源码入口、当前阶段、最终目标和下一步；但没有重复维护 `knowledge.md`、`commands.md`、`cases.md` 或源码侧 README/实施计划的详细内容。
