...
- `context.md`：会话记忆加范围边界。记录接手摘要、当前目标、当前阶段、已验证边界、阻塞、下一步、开放风险和过期条件。它是短期工作台和恢复入口，不是长期事实。
...
- `context.md` 维护接手摘要、当前目标、当前阶段、阻塞、下一步、开放风险和过期条件。接手摘要用于让新会话或接手人先定位现场，应包含工作根目录、相关子仓库或工作区、当前分支/commit、涉及源码目录、源码侧说明文档路径、最终目标、阶段性完成目标和立即下一步。只写导航级摘要，不复制 README、实施计划、命令全文或案例细节。
...
## 接手摘要
工作根目录、相关子仓库或工作区、当前分支/commit、涉及源码目录、源码侧说明文档路径、最终目标、阶段性完成目标和立即下一步。只放接手所需的导航级摘要；详细事实、命令、案例和项目文档分别放到对应主记录。
