...
- `context.md`：会话记忆加范围边界。记录接手摘要、最终目标、当前目标、当前阶段、已验证边界、阻塞、下一步、开放风险和过期条件。它是短期工作台和恢复入口，不是长期事实。
...
- `context.md` 维护接手摘要、最终目标、当前目标、当前阶段、阻塞、下一步、开放风险和过期条件。接手摘要用于让新会话或接手人先定位现场，使用固定表格记录工作根目录、相关子仓库或工作区、当前分支/commit、主要工作目录或源码目录、关键入口或文件、相关说明/计划/证据路径、当前阶段和立即下一步。最终目标单独成节，避免被埋在摘要里。只写导航级摘要，不复制 README、实施计划、命令全文、原始日志或案例细节。
...
| --- | --- |
| 工作根目录 | `<path>` |
| 相关仓库/工作区 | `<repo-or-worktree>` |
| 当前分支/commit | `<branch>` / `<commit>` |
| 主要工作/源码目录 | `<path>` |
| 关键入口/文件 | `<paths>` |
| 相关说明/计划/证据 | `<paths>` |
| 当前阶段 | `<phase>` |
...
只放接手所需的导航级摘要；详细事实、命令、案例、原始日志和项目文档分别放到对应主记录。
...
- `context.md` 可以让新会话快速恢复现场：知道工作目录、工作入口或源码入口、当前阶段、最终目标和下一步；但没有重复维护 `knowledge.md`、`commands.md`、`cases.md` 或相关 README/实施计划/证据文件的详细内容。
